...
#include "raylib.h"
#include "raymath.h"
#include <stdint.h> // clay.h use this
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
                                       ...
                                       float zDistance)
{
	...
	if (camera.projection == CAMERA_PERSPECTIVE)
	{
		...
		matProj =
		    MatrixPerspective(camera.fovy * DEG2RAD, ((double)screenWidth / (double)screenHeight), 0.01f, zDistance);
	}
...
void Clay_Raylib_Render(Clay_RenderCommandArray renderCommands, Font *fonts)
{
	for (int j = 0; j < renderCommands.length; j++)
	{
		...
		Clay_BoundingBox boundingBox = {roundf(renderCommand->boundingBox.x), roundf(renderCommand->boundingBox.y),
		...
		case CLAY_RENDER_COMMAND_TYPE_TEXT: {
			...
			// Raylib uses standard C strings so isn't compatible with cheap slices, we need to clone the string to
			// append null terminator
			memcpy(temp_render_buffer, textData->stringContents.chars, textData->stringContents.length);
			temp_render_buffer[textData->stringContents.length] = '\0';
			DrawTextEx(fontToUse, temp_render_buffer, (Vector2){boundingBox.x, boundingBox.y},
		...
		case CLAY_RENDER_COMMAND_TYPE_IMAGE: {
			...
			DrawTexturePro(imageTexture, (Rectangle){0, 0, imageTexture.width, imageTexture.height},
		...
		case CLAY_RENDER_COMMAND_TYPE_RECTANGLE: {
			...
			if (config->cornerRadius.topLeft > 0)
			{
				...
				DrawRectangleRounded((Rectangle){boundingBox.x, boundingBox.y, boundingBox.width, boundingBox.height},
		...
		case CLAY_RENDER_COMMAND_TYPE_BORDER: {
			...
			if (config->cornerRadius.topLeft > 0)
			{
				DrawRing((Vector2){roundf(boundingBox.x + config->cornerRadius.topLeft),
				         ...
				         270, 10, CLAY_COLOR_TO_RAYLIB_COLOR(config->color));
			}
			if (config->cornerRadius.topRight > 0)
			{
				DrawRing((Vector2){roundf(boundingBox.x + boundingBox.width - config->cornerRadius.topRight),
				         ...
				         360, 10, CLAY_COLOR_TO_RAYLIB_COLOR(config->color));
			}
		...
		case CLAY_RENDER_COMMAND_TYPE_CUSTOM: {
			...
			case CUSTOM_LAYOUT_ELEMENT_TYPE_3D_MODEL: {
				...
				float scaleValue =
				    CLAY__MIN(CLAY__MIN(1, 768 / rootBox.height) * CLAY__MAX(1, rootBox.width / 1024), 1.5f);
